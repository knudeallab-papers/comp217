...
public class dayRecordpage extends JFrame {
	...
	private JTextField weight_textField;					// 몸무게 입력 필드
	private JPanel ex_list_panel; 							// 운동기록들 나타내는 패널
	private ArrayList<expanel> expanel_list;				// ex_list_panel에 들어갈 expanel들 리스트 
	public dayRecord dayrecord;								// 선택한 날의 dayRecord 정보 - 정보 입력과 출력에 사용
	...
	public dayRecordpage(final ArrayList<dayRecord> dR_ary,dayRecord dr) {//이부분 final로 안하니 오류 떠서 final 추가했습니다(동혁)
		...
		// 기존 정보 있는지에 따라 분기
		dayrecord = dr;
		if(dr.getExr_ary().size()>0) {						// 만약 dr의 ex_ary가 차있다면 expanel_list에 기존내용 추가해줘야함
			exist = true;
		...
		ActionListener addex_listener= new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(exrp.exit) { //정상종료
					exRecord tmp_ex = new exRecord(exrp.get_exname(),exrp.get_setgoal());					// 받아온 운동 정보 임시 저장
					dayrecord.add_exr(tmp_ex);																//dayrecord에 받아온 정보 저장
					expanel tmp_exp = new expanel(tmp_ex);													// 받아온 정보를 바탕으로 panel생성
	...
	// ex_list_panel에 expanel_list에 들어있는 모든 expanel 새로 넣어서 repaint하는 method
	...
	class expanel extends JPanel{ 
		...
		public expanel(exRecord other_exr) {
			...
			// 운동명 Label
			ex_name = new JLabel(exr.getEx().getname());
			...
			setnum_label = new JLabel("("+Integer.toString(exr.getCount_set())+"/"+Integer.toString(exr.getSet_goal())+")");
		...
		// 현재 expanel의 index 위치를 반환하는 method
