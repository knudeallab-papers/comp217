...
public class ChatServer implements Runnable {
            ...
            public void run() {
                while (running) {
                    ...
                    for (int i = 0; i < clients.size(); i++) {
                        ChatServerClient c = clients.get(i);
                        if (!clientResponse.contains(c.getID())) {
                            ...
                            clientResponse.remove(valueOf(c.getID()));
                            c.attempt = 0;
                        }
                    }
                }
    ...
    //For sending out messages to all clients
    private void sendToAll(String message) {
        ...
        for (int i = 0; i < clients.size(); i++) {
            ChatServerClient client = clients.get(i);
    ...
    private void process(DatagramPacket packet) {
        ...
        if (string.startsWith("/c/")) {
            ...
            clients.add(new ChatServerClient(name, packet.getAddress(), packet.getPort(), id));
            String ID = "/c/" + id;
