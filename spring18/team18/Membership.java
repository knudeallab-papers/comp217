...
public class Membership extends JFrame{
	...
	public static final int EDIT_HEIGHT = 500;
	public static final int ADD_HEIGHT = 400;
	...
	private JLabel getDataOfSelectedRow[] = new JLabel[5];
	private JTable stockList;
	private String header[] = {"Number", "Grade", "Name", "Mileage", "Phone"};		//테이블 열 제목;
	private String[][] mData = new String[50][5];		//테이블 내부 정보 저장 배열;
	...
	public   static String Save[][]=new String[20][5];


	public Membership(){
		
		super("membership Demo");
		...
		stockList.addMouseListener((MouseListener) new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if(selectedRow<0) return;
					...
					if(stockList.getValueAt(selectedRow, 0) != null) {
						for(int k=0; k<4; k++) {
							getDataOfSelectedRow[k].setText((String)stockList.getValueAt(selectedRow, k+1));
						}
					}
				}
			}
		...
		detailDataPane.setLayout(new GridLayout(4, 2));
		detailDataPane.setBorder(new LineBorder(Color.BLACK,1));
		
		JLabel detailDataLabels[] = new JLabel[4];
		detailDataLabels[0] = new JLabel("    Grade : ");
		detailDataLabels[1] = new JLabel("    Name  : ");
		detailDataLabels[2] = new JLabel("    Mileage  : ");
	...
	protected void resetGrade(int a) {
		if(a == 1) {
	...
	protected void getMemberFile() {
		Scanner inputStream = null;
		ARRAY_COUNT = 0;
		File file = new File("membership.txt");
		if(!file.exists()) {
			//System.out.println("Cannot found membership file");
			return;
		}
		...
		while(inputStream.hasNextLine()) {
			mData[ARRAY_COUNT][0] = inputStream.next();
	...
	public  void FileIo () throws IOException {
       ...
       {
		reader.write(mData[j][0]+"\t"+mData[j][1]+"\t"+mData[j][2]+"\t"+mData[j][3]+ "\t" + mData[j][4] + "\n"); // 파일에 "입출력!"을 저장함.
