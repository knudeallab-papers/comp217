...
public class Employee2 extends JFrame implements MouseListener,ActionListener{
	JTable table;
	JScrollPane scroll;
    ...
    Object[][] rowData={
                               {1,"홍길동", 10000, "메니저 ","2011/5/4","010-2354-3567"}, //0행
                               {2,"닐로 ", 6500, "알바","2017/2/3","010-2124-2938"},//1행
                               {3,"장덕", 6500, "알바","2014/9/4","010-5938-2085"},  //2행
                               
              
                                };
    Object[] columnNames={"번호","이름","급여","직급","입사","연락"};
 ...
 @Override
	...
	//  System.out.println(e.getActionCommand());
	  ...
	  if(ob==bt_add){//추가버튼 클릭시
	   ...
	   form.setVisible(true);
	      ...
	      String day= form.tf_day.getText();
	      ...
	      if(day.length()==0){
		      ...
		      JOptionPane.showMessageDialog(form, "입사일 입력!!"); 
		          form.tf_day.requestFocus();
		       return;
		      }
	      if(phone.length()==0){
		       //TextField에는 null값이 들어오지 않으므로 null체크 뺌.
		      JOptionPane.showMessageDialog(form, "연락 입력!!"); 
		          form.tf_phone.requestFocus();
		       return;
		      }
	      
	      if(form.getTitle().equals("입력폼")){//입력폼이었을때
	      Object rowData[] = {number,name, pay,pos, day,phone};
	      dtm.addRow(rowData);
	      }else{//수정폼이였을때
	//       table.setValueAt(Object aValue, int row, int column)
	       ...
	       table.setValueAt(day, srow, 4);
	       table.setValueAt(phone, srow, 5);
	      }
	...
	//     table.getValueAt(row index, column index);
	  ...
	  String pay  = table.getValueAt(srow,2).toString();
	                    //nw Integer(13);
	  String pos  = (String) table.getValueAt(srow,3);
	  String day  = (String) table.getValueAt(srow,4);
